...
A standalone Go TUI application and module with a consumable public package.
Not a bento in the bentotui registry. A complete, shippable terminal diff viewer — the first
proof-of-platform tool built entirely on BentoTUI, the same way Charm builds
tools with their own stack.

Consumption modes:
- Application mode via `cmd/bento-diffs`
- Library mode via direct module import `github.com/cloudboy-jh/bento-diffs`
...
bento-diffs/
  cmd/
    bento-diffs/
      main.go                    ← CLI entry: reads input, runs tea.NewProgram
  go.mod
  go.sum
  README.md
  CHANGELOG.md
...
```bash
# Run locally
git diff | go run ./cmd/bento-diffs
go run ./cmd/bento-diffs before.go after.go

# Build binary
go build -o bento-diffs ./cmd/bento-diffs

# Install CLI
go install github.com/cloudboy-jh/bento-diffs/cmd/bento-diffs@latest
```
